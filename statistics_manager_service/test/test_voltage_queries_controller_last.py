...
class TestVoltageQueriesControllerLast(BaseTestCase):
    """VoltageQueriesController for last values integration test stubs"""

    def test_voltage_last_get(self):
        """Test case for voltage_last_get

        Retrieve the last value of the voltage for one user.
        """
        clean_account()
        ...
        self.assertEqual(metrics_dict["units"], "V")
        self.assertEqual(metrics_dict["value"], 879.4)

    def test_voltage_last_get_several_users_and_no_api_key(self):
        """Test case for voltage_last_get for several users

        Retrieve the last value of the voltage for several users.
        ...
        self.assertEqual(datetime.fromisoformat(metrics[good_user_id_index]["timestamp"]), datetime(
            2022, 12, 23, 13, 0, 51, tzinfo=timezone.utc))
        self.assertEqual(metrics[good_user_id_index]["units"], "V")
        ...
        self.assertEqual(metrics[bad_user_id_index]["units"], "V")
        self.assertTrue(math.isnan(metrics[bad_user_id_index]["value"]))

    def test_voltage_last_get_empty(self):
        """Test case for voltage_last_get with an empty value on DB

        Retrieve the last value of the voltage for several users but there is no value on DB.
        """
        clean_account()
    ...
    def test_voltage_wrong_userid(self):
        """Test case for voltage_last_get with a non existing user_id

        Retrieve the last value of the voltage for several users with a non existing user_id
        """
        clean_account()
    ...
    def test_voltage_last_get_several_users_with_authentication(self):
        """Test case for voltage_last_get for several users with authentication
        Meaning that the user will not have access to the other user_id

        Retrieve the last value of the voltage for several users with authentication
